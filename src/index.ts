...
export default {
  async scheduled(
    ...
    if (env.DISCORD_API_TOKEN === null || env.DISCORD_API_TOKEN.length === 0) {
      const message = "Failed to find Discord API token";
      sentry.captureException(message);
      throw new Error(message);
    }
    if (env.DISCORD_GUILD_ID === null || env.DISCORD_GUILD_ID.length === 0) {
    ...
    const events_to_message = scheduled_events_json.filter((event) => {
      ...
      if (
        ...
        console.log(`Skipping event ${event.name} since it's not tomorrow`);
        return false;
      }
    });
  ...
  async message(
    ...
    const event_time = new Date(event.scheduled_start_time).toLocaleTimeString(
      "en-US",
      {
        timeStyle: "short",
